class DiceWFRP {
    ...
    static rollTest(testData){
      ...
      {
        description = "Success"
        if (game.settings.get("dh", "fastSL"))
        {
        ...
        if (game.settings.get("dh", "testAbove100"))
        {
          if (targetNum > 100)
          {
      ...
      if (game.settings.get("dh", "criticalsFumblesOnAllTests") && !testData.hitLocation)
      {
        if (roll.total > targetNum && roll.total % 11 == 0 || roll.total == 100)
        {
          rollResults.extra.color_red = true;
    ...
    // Extend rollTest for casting specifics (miscasts, CN, etc)
     static rollCastTest(testData){
      ...
      if (game.settings.get("dh", "partialChannelling"))
      {
      ...
      testData.function = "rollChannellTest"
       ...
       if (testData.extra.malignantInfluence)
        ...
        if (testResults.description.includes("Failure")) // Failed Test
        {
          ...
          if (Number(SL) == 0 && game.settings.get("dh", "extendedTests"))
            SL = -1;
  ...
  */
   static renderRollCard(chatOptions, testData, rerenderMessage) {
    ...
    if (game.settings.get("dh", "manualChatCards") && !rerenderMessage)
    {
    ...
    if (!rerenderMessage)
    {
      ...
      return renderTemplate(chatOptions.template, chatData).then(html => {
        ...
        if (game.settings.get("dh", "manualChatCards"))
        {
          let blank = $(html)
    ...
    static chatListeners(html) {
      ...
      html.on('mousedown', '.table-click', ev => {
        ...
        {
          renderTemplate('systems/dh/templates/chat/table-dialog.html').then(html => {
            new Dialog({
