...
class ItemSheetWfrp4e extends ItemSheet {
  ...
  async _render(force = false, options = {})
  {
    ...
    get template() {
      ...
      return `systems/dh/templates/items/item-${type}-sheet.html`;
    }
      ...
      html.find('.lore-input').change(async event => {
        ...
        for (let lore in WFRP4E.magicLores)
        {
          if (inputLore == WFRP4E.magicLores[lore])
          {
           ...
           // await this.item.update({'img' : `systems/dh/icons/spells/${lore}.png`})
...
try 
{
  ...
  Items.registerSheet("dh", ItemSheetWfrp4e, {makeDefault: true});
}
