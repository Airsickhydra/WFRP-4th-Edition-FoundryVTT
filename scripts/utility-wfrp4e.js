...
class WFRP_Utility
{
  ...
  {
    let skillList = [];
    let pack = game.packs.find(p => p.collection == "dh.skills")
  ...
  {
    let talentList = [];
    let pack = game.packs.find(p => p.collection == "dh.talents")
  ...
  static displayStatus(tokenId, round = undefined)
  {
    ...
    chatOptions["template"] = "systems/dh/templates/chat/combat-status.html"
  ...
  static displayRoundSummary(combat)
  {
    ...
    if ( ["gmroll", "blindroll"].includes(chatOptions.rollMode) ) chatOptions["whisper"] = ChatMessage.getWhisperIDs("GM");
    if ( chatOptions.rollMode === "blindroll" ) chatOptions["blind"] = true;
    chatOptions["template"] = "systems/dh/templates/chat/round-summary.html"
